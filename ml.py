...
class ML_Trend(Strategy):
    ...
    def initialize(self):
        ...
        self.data = self.get_historical_prices(selected_symbol, 51, "day").df
...
if __name__ == "__main__":
    ...
    trade =False
    backtest=False
    ALPACA_CONFIG = {#saving everything in one variable
    'API_KEY' : os.getenv("APCA_API_KEY_ID"),
    'API_SECRET' : os.getenv("APCA_API_SECRET_KEY"),
    "ENDPOINT": os.getenv("APCA_API_BASE_URL")
    } 

    restAPI=tradeapi.REST(key_id=ALPACA_CONFIG['API_KEY'],secret_key=ALPACA_CONFIG['API_SECRET'],base_url=ALPACA_CONFIG['ENDPOINT'])
    # Get the list of all tradable assets
    tradable_symbols = [asset.symbol for asset in restAPI.list_assets() if asset.tradable]

    # Display symbols in a numbered list
    print("Available tradable symbols:")
    for idx, symbol in enumerate(tradable_symbols[:], 1):  # Show first all symbols
        print(f"{idx}. {symbol}")

    # Let the user choose a symbol
    while True:
        try:
            choice = int(input("\nEnter the number corresponding to the symbol you want to trade: "))
            if 1 <= choice <= len(tradable_symbols):
                selected_symbol = tradable_symbols[choice - 1]
                print(f"\nYou have selected: {selected_symbol}")
                if(input(f"\nDo You want to trade {selected_symbol}?(Y/N):").strip().lower()=='y'):
                    trade=True
                elif(input(f"\nDo You want to perfrom backtesting?(Y/N):").strip().lower()=='y'):
                    backtest=True
                break
            else:
